...
void outputTree( std::ofstream & out, ctBranch * b, unsigned char * data, int thresh ) {
    out << "(" << b->extremum << " <" << (int)data[b->extremum] << "> " << b->saddle << " <" << (int)data[b->saddle] << ">";
    printf("(%d <%d> %d <%d>", b->extremum, data[b->extremum], b->saddle, data[b->saddle]);    
    
    for ( ctBranch * c = b->children.head; c != NULL; c = c->nextChild ){
      if ( abs(int(data[c->extremum])-int(data[c->saddle])) > thresh ) {
        out << " ";
        printf(" ");
        //printf("%d %d\n",  data[c->extremum], data[c->saddle]);
...
/*
void outputContourTree( std::ofstream & out, ctArc * a, unsigned char * data, int thresh ) {

   //Find maximum height node
   ctNode* hi;
   while(){
     size_t value = current->hi->t;
     if(value > hi->t)
       hi = current->hi;
...
int main( int argc, char ** argv ) 
{
    ...
    //create contour tree
    ctArc* arc = ct_sweepAndMerge( ctx );
    ...
    if (out) {
            ...
            //outputContourTree(out, arc, data, thresh);
    } else {
            cerr << "ERR: couldn't open output file " << outfile << endl;
    }

    //See: http://graphics.cs.ucdavis.edu/~sdillard/libtourtre/doc/html/tourtre_8h.html#35ca85eb51b3f9d381610fe757435374
